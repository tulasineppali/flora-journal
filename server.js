...
app.get('/api/summary', (_req, res) => {
  ...
  const totalEntries = entries.length, totalSightings = sightings.length, totalPhotos = photos.length;
  ...
  const mostPhotographed = Object.entries(photosBySighting).sort((a,b) => b[1]-a[1]).slice(0,5)
    .map(([sid,count]) => ({ name: sightingById[sid]?.name || 'Unknown', count }));

  res.json({
    empty: false, totalEntries, totalSightings, totalPhotos, uniqueDays,
    avgSightings: parseFloat(avgSightings), uniqueSpecies, rareSightings,
    firstEntry: dated[0]     ? { date: dated[0].date,     location: dated[0].location }     : null,
    lastEntry:  dated.at(-1) ? { date: dated.at(-1).date, location: dated.at(-1).location } : null,
...
// Pre-seed last sync date if user has already manually imported up to a point
{
  ...
  if (!existing) {
    ...
    console.log('ℹ️  iNaturalist: last sync pre-seeded. Next sync will only fetch newer observations.');
  }
}
app.listen(PORT, () => {
